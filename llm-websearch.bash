...
for link in "${links[@]}" ; do
   ...
   if [[ "${ans}" == Yes* ]] ; then
      ...
      if [[ "${type}" == "[Pp][Dd][Ff]" ]] ; then
         ...
         curl -s -L -A 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/118.0.0.0 Safari/537.36' "${lurl}" | html2text > /dev/shm/llm-websearch.txt
      fi

      #Ask the LLM to check the text for helpful information and summarize it.
      pans=$(llm-python-file.py /dev/shm/llm-websearch.txt "You are a helpful research assistant." "We are trying to research \`${sterm}\` and we have a webpage with the url \`${lurl}\` that has the following text:" "Summarize the portions of the page that help answer \`${sterm}\`.  Only directly summarize the text to help the research and nothing else." "0.7" | sed -e 's/\\n/ /g' -e "s/*//g" -e 's/\\//g' | tr -d '\n')
